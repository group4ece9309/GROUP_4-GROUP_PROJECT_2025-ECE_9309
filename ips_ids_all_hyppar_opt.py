...
# Sample 50% of the dataset to speed up processing
...
### 🔹 Hyperparameter Optimization for All Models (Minimal Computation) ###
...
def objective_gnb(trial):
    var_smoothing = trial.suggest_float("var_smoothing", 1e-9, 1e-6, log=True)  # FIXED
...
def objective_lr(trial):
    C = trial.suggest_float("C", 0.01, 10, log=True)  # FIXED
...
def objective_gbc(trial):
    ...
    learning_rate = trial.suggest_float("learning_rate", 0.01, 0.2, log=True)  # FIXED
...
study_gbc.optimize(objective_gbc, n_trials=3)
...
best_models = {
    ...
    "Gradient Boosting": GradientBoostingClassifier(**study_gbc.best_params, random_state=42),
...
# Convert results to DataFrame & Plot
